...
from selenium.webdriver.chrome.service import Service  # edited!!
...
def scrape_centanet():
    try:
        ...
        options = Options()
        options.binary_location = os.environ.get("GOOGLE_CHROME_BIN", "/usr/bin/google-chrome")  # edited!!
        ...
        service = Service(os.environ.get("CHROMEDRIVER_PATH", "/usr/bin/chromedriver"))  # edited!!
        driver = webdriver.Chrome(service=service, options=options)  # edited!!
        ...
        for card in listings[:5]:  # limit to 5 for safety
            try:
